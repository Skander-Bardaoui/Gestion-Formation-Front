...
export function ChatBot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "bot",
      text: `Bonjour${isAuthenticated && user ? ` ${user.prenom || user.username}` : ""} ! Je suis l'assistant StirForma. ${!isAuthenticated ? "Connectez-vous pour profiter de mes services." : getRoleMessage(user?.role)}`,
    },
  ]);
        ...
        <div className="fixed bottom-24 right-6 z-50 flex w-80 flex-col rounded-2xl border border-border bg-card shadow-2xl sm:w-96">
          <div className="flex items-center gap-2 border-b border-border px-4 py-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-foreground">Assistant StirForma</p>

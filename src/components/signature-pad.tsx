...
export function SignaturePad({ onSave, width = 500, height = 180, label = "Signez ici", savedSignature }: SignaturePadProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative overflow-hidden rounded-lg border-2 border-dashed border-border bg-background">
        {mode === 'upload' && !hasContent && (
          ...
          >
            ...
            <span className="text-xs text-muted-foreground/60">Cliquez pour sélectionner une image (PNG, JPG)</span>
            <input
              ref={fileInputRef}
              type="file"
        ...
        <canvas
          ...
          style={{ maxWidth: width, height, display: mode === 'upload' && !hasContent ? 'none' : 'block' }}
      ...
      {mode === 'upload' && hasContent && (
        <p className="text-xs text-muted-foreground">Aperçu de l'image importée. Vous pouvez l'effacer ou la valider.</p>
      )}

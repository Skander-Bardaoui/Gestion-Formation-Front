...
import { Calendar, ArrowRight, Loader2, BookOpen, Star, Wallet, Clock, ChevronDown, ChevronUp, Paperclip, FileText, Download, Award } from "lucide-react";
...
import { getMyCertificates, getCertificateDownloadUrl, type Certificate } from "@/lib/api/certificates";
            ...
            <div className="mt-8">
              ...
              <div className="space-y-3">
                {pendingInscriptions.map((ins) => {
                  ...
                  return (
                    <div key={ins.id} className="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-6">
    ...
    <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="flex gap-4">
          {session.formation?.imageUrl ? (
            ...
            <div className="hidden sm:flex shrink-0 w-32 h-20 overflow-hidden rounded-lg bg-gradient-to-br from-primary/10 to-secondary/10 items-center justify-center">
              <span className="font-display text-2xl text-muted-foreground/20">{session.formation?.titre?.[0] || "F"}</span>
            </div>
          )}
          <div>
            <div className="flex items-center gap-2">
      ...
      {expanded && (
        <div className="mt-3 space-y-2">
          {documents?.length === 0 && (
            <p className="text-center text-xs text-muted-foreground">Aucun document pour cette session</p>
          )}
          {documents?.map((doc: SessionDocument) => (
            <a
              key={doc.id}
              href={`http://localhost:3001/api/sessions/${session.id}/documents/${doc.id}/download?token=${token}`}
      ...
      {sessionCerts.length > 0 && (
        <div className="mt-3">
          ...
          >
            <span className="flex items-center gap-2"><Award className="h-4 w-4" /> Certificats</span>
            ...
            <div className="mt-2 space-y-2">
              {sessionCerts.map((cert: Certificate) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">{new Date(cert.dateEmission).toLocaleDateString("fr-FR")}</span>
                  <Download className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                </a>
              ))}
            </div>

            ...
            <dl className="mt-12 grid max-w-lg grid-cols-3 gap-6">
              {[
                ...
                <div key={s.v}>
                  ...
                  <dd className="mt-1 text-xs uppercase tracking-wide text-muted-foreground">{s.v}</dd>
                </div>
              ))}
            </dl>
          </div>
          ...
          <div className="md:col-span-5">
            <div className="relative rounded-2xl border border-border bg-card p-6 shadow-[0_30px_80px_-40px_oklch(0.32_0.07_165/0.4)]">
              <div className="flex items-center justify-between border-b border-border pb-4">
                <div>
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">Prochaine session</p>
                  <p className="font-display text-2xl">Cybersécurité — Fondamentaux</p>
                </div>
                <span className="rounded-md bg-ochre/20 px-2 py-1 text-xs font-medium text-ochre-foreground">Inter</span>
              </div>
              <div className="mt-5 space-y-3 text-sm">
                ...
                <Row icon={<GraduationCap className="h-4 w-4" />} label="Idriss Bennani, expert SSI" />
                <Row icon={<ShieldCheck className="h-4 w-4" />} label="Certificat signé électroniquement" />
              </div>
              <div className="mt-5 rounded-lg bg-secondary p-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs uppercase tracking-wide text-muted-foreground">Présences du jour</span>
        ...
        <div className="mx-auto max-w-7xl px-6 py-20">
          <div className="grid gap-10 md:grid-cols-12 md:gap-16">
            ...
            <div className="grid gap-6 md:col-span-8 md:grid-cols-2">
              {features.map((f) => (
                <article key={f.title} className="rounded-xl border border-border bg-background p-6 transition-all hover:-translate-y-0.5 hover:shadow-lg">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24">
        <div className="relative overflow-hidden rounded-3xl bg-primary px-8 py-16 text-primary-foreground md:px-16">
          ...
          <div className="relative grid gap-8 md:grid-cols-2 md:items-center">
            <div>
              <h2 className="font-display text-4xl md:text-5xl">Prêt à digitaliser votre service formation ?</h2>
...
const features = [
  { icon: BookOpen, title: "Formations Intra · Inter · Catalogue", desc: "Configurez chaque session selon son format, ses participants, son lieu et ses livrables." },
  { icon: Calendar, title: "Planning & convocations", desc: "Calendrier intégré, invitations automatiques et rappels e-mail aux participants." },
  { icon: Users, title: "Suivi des participants", desc: "Historique de formation, certifications obtenues et fiches de présence en un clic." },
  { icon: ShieldCheck, title: "Conformité & signatures", desc: "Signature électronique des certificats, QR-code de vérification, RGPD natif." },
];

...
export const Route = createFileRoute("/catalogue")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Toutes les formations Atelier : design, développement, data, management." },
    ],
  }),
...
function CataloguePage() {
  ...
  const categories = useMemo(
    () => ["Toutes", ...Array.from(new Set((formations ?? []).map((f) => f.categorie).filter(Boolean)))],
    [formations]
  );
    ...
    <PageShell>
      <section className="mx-auto max-w-7xl px-6 pt-16 pb-10">
        ...
        <h1 className="mt-3 font-display text-5xl md:text-6xl">
          {isLoading ? "..." : `${filtered.length} formation${filtered.length > 1 ? "s" : ""} à explorer`}
        </h1>
            ...
            {categories.map((c) => (
              <button
                ...
                className={`rounded-full border px-4 py-1.5 text-sm transition ${
                  cat === c ? "border-primary bg-primary text-primary-foreground" : "border-border hover:border-foreground/40"
                }`}
              >
                {c}
              </button>
            ...
            {filtered.map((f) => (
              ...
              >
                <div className="aspect-[5/3] overflow-hidden bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center">
                  {f.imageUrl ? (
                    <img src={`http://localhost:3001${f.imageUrl}`} alt={f.titre} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
                  ) : (
                ...
                <div className="flex flex-1 flex-col p-6">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span className="capitalize">{f.type} · {f.dureeEnJours}j</span>
                  </div>

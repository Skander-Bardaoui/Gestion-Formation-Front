...
export const registerSchema = z.object({
  username: z.string().min(3, "Minimum 3 caractères").regex(/^[a-zA-Z0-9._-]+$/, "Lettres, chiffres, . _ - uniquement"),
  email: z.string().email("Email invalide"),
  password: z.string().min(6, "Minimum 6 caractères"),
  confirmPassword: z.string(),
  role: z.enum(["participant", "formateur"]),
...
export const employeSchema = z.object({
  nom: z.string().min(2, "Minimum 2 caractères").regex(/^[a-zA-ZÀ-ÿ\s-]+$/, "Caractères invalides"),
  prenom: z.string().min(2, "Minimum 2 caractères").regex(/^[a-zA-ZÀ-ÿ\s-]+$/, "Caractères invalides"),
  email: z.string().email("Email invalide"),
  poste: z.string().optional(),
  departement: z.string().optional(),
  telephone: z.string().regex(/^(\+216)?\d{8}$/, "Numéro invalide (ex: +216XXXXXXXX)").optional().or(z.literal("")),
